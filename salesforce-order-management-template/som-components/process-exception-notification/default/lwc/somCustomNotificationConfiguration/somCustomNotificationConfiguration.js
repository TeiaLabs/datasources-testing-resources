...
import getCustomNotificationTypeList from '@salesforce/apex/SOM_ExceptionNotificationHelper.getCustomNotificationTypeList';
...
const FIELDS = [
    ...
    'SOM_PEConfiguration__c.CustomNotificationMessage__c',
    'SOM_PEConfiguration__c.CustomNotificationTitle__c',
    'SOM_PEConfiguration__c.CustomNotificationTypeId__c',
...
const customLabelsArray = [
    ...
    {'SOM_CustomNotificationConfigurationCardTitle' : getCustomLabels().SOM_CustomNotificationConfigurationCardTitle},
    {'SOM_CustomNotificationConfigurationMessageLabel' : getCustomLabels().SOM_CustomNotificationConfigurationMessageLabel},
    {'SOM_CustomNotificationConfigurationNotificationTypeLabel' : getCustomLabels().SOM_CustomNotificationConfigurationNotificationTypeLabel},
    {'SOM_CustomNotificationConfigurationNotificationTypePlaceHolder' : getCustomLabels().SOM_CustomNotificationConfigurationNotificationTypePlaceHolder},
    {'SOM_CustomNotificationConfigurationTitleLabel' : getCustomLabels().SOM_CustomNotificationConfigurationTitleLabel},
    {'SOM_Save' : getCustomLabels().SOM_Save},
    {'SOM_SaveAs' : getCustomLabels().SOM_SaveAs},
    {'SOM_Cancel' : getCustomLabels().SOM_Cancel}
];  

export default class SomCustomNotificationConfiguration extends LightningElement {
    ...
    titleChange(event) {
        this.notificationTitle = event.detail.value;
        this.isChanged = true;
    }
    // Change Notification message
    messageChange(event) {
        this.notificationMessage = event.detail.value;
        this.isChanged = true;
    }
    // Query custom notification options
    @track notificationType;
    @track customNotificationTypes;
    _customNotificationTypesWireData
    @wire(getCustomNotificationTypeList) 
    getCustomNotificationTypes(notificationsData) {
        this._customNotificationTypesWireData = notificationsData;
        const { data, error } = notificationsData;
        if (data) {
            this.customNotificationTypes = data.map(x => ({ label: x.CustomNotifTypeName, value: x.Id }));
        }else if (error) {
            let event = showToastForWireError(error, 'Custom Notification Types could not be received');
            this.dispatchEvent(event);
        }
    }
    // Notification types
    get getNotificationTypes() {
        return this.customNotificationTypes;
    }
    ...
    @wire(selectProcessExceptionConfigurationByConfigurationTypeAndSalesChannel, { configuratonType: 'CustomNotification', salesChannelId: '$saleschannelid' }) 
    ...
    getCurrentConfiguration(currentConfigurationRecord) {
        ...
        if(data){
            ...
            this.notificationMessage = data.fields.CustomNotificationMessage__c.value;
            this.notificationTitle = data.fields.CustomNotificationTitle__c.value;
            this.notificationType = data.fields.CustomNotificationTypeId__c.value;
            this.toggleButtonLabel = this.template.querySelector(".isConfigEnabled").checked ? 'Enabled' : 'Disabled';
    ...
    getFieldsOfNewRecord(){
        return {
            'ConfigurationType__c' : 'CustomNotification',
            'SalesChannelId__c' : this.saleschannelid,
            'IsEnabled__c' : this.toggleButtonLabel === 'Enabled',
            'Recipients__c' : this.recipientIds.toString(),
            'CustomNotificationMessage__c': this.notificationMessage,
            'CustomNotificationTitle__c': this.notificationTitle,
            'CustomNotificationTypeId__c': this.notificationType,
    ...
    updateConfigurationRecord(){
        ...
        updateRecord(objRecordInput).then(response => {
            ...
            refreshApex(this._getConfigurationRecordWireData);
            refreshApex(this._getCurrentConfigurationWireData);
            refreshApex(this._customNotificationTypesWireData);
        }).catch(error => {
